            ...
            <table className="w-full text-left text-sm text-slate-600">
              <thead className="text-xs text-slate-500 bg-slate-50 border-b border-slate-200">
                <tr>
                  ...
                  <th className="px-4 py-3 font-semibold uppercase">Cursos Vinculados</th>
                      ...
                      <td className="px-4 py-3">
                        <div className="flex flex-wrap gap-1">
                          {exam.course_ids && exam.course_ids.length > 0 ? (
                            exam.course_ids.map((cid: string) => {
                              const courseTitle = courses.find(c => c.id.toString() === cid)?.title || cid;
                              return (
                                <span key={cid} className="bg-slate-100 text-slate-600 px-2 py-0.5 rounded text-[10px] font-bold">
                                  {courseTitle}
                                </span>
                              );
                            })
